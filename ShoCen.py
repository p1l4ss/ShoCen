...
def get_shodan_data(hostname):
    ...
    try:
        results = api.search('ssl:' + hostname) |  api.search('hostname:'+hostname)
        if results['matches']:
            extracted_data = []
...
def main():
    ...
    parser.add_argument("-d", "--domains", required=True, nargs='+', help="Domains to search for")
    parser.add_argument("-o", "--output", help="Output file to save JSON results")
    parser.add_argument("-t", "--threads", type=int, default=10, help="Number of threads to use (default: 10)")
    args = parser.parse_args()

    combined_data = get_combined_data(args.domains, max_threads=args.threads)
...
#python3 ShoCen.py -d petrolimex.com.vn -t 10 -o results.json
